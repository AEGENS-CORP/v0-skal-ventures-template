...
export class DofPointsMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      ...
      uniform float uTime;
      uniform vec2 uMousePosition;
      uniform vec2 uBackgroundClickCenter;
      uniform float uHasBackgroundClick;
      uniform float uBackgroundClickProgress;
      varying float vDistance;
      varying float vPosY;
      varying vec3 vWorldPosition;
      varying vec3 vInitialPosition;
      ...
      void main() {
        ...
        if (uHasBackgroundClick > 0.5) {
          // Use background click position as center (convert from normalized to world space)
          vec2 clickWorldPos = uBackgroundClickCenter * 5.0; // Scale to match world space
          distanceFromCenter = length(vWorldPosition.xz - clickWorldPos);
        } else {
          // Use default center (0,0)
          distanceFromCenter = length(vWorldPosition.xz);
        }
        ...
        if (uHasBackgroundClick > 0.5) {
          revealMask *= uBackgroundClickProgress;
        }
        
        // Calculate sparkle brightness multiplier
        float sparkleBrightness = sparkleNoise(vInitialPosition, uTime);
        
        float mouseProximityBoost = 1.0 + (1.0 - smoothstep(0.0, 3.0, vMouseDistance)) * 0.8;
        sparkleBrightness *= mouseProximityBoost;
        ...
        gl_FragColor = vec4(vec3(0.6), mix(alpha, sparkleBrightness - 1.1, uTransition));
      }`,
      uniforms: {
        ...
        uBackgroundClickCenter: { value: new THREE.Vector2(0, 0) },
        uHasBackgroundClick: { value: 0.0 },
        uBackgroundClickProgress: { value: 0.0 }, // Added smooth progress uniform
      },
      transparent: true,
      depthWrite: false,
    })
